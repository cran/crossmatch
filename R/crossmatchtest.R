...
if (!isSymmetric(D)){stop("Invalid distance matrix: your distance matrix is not symmetric"); return(NA)}
if (sum(D<0)>0){stop("Invalid distance matrix: your distance matrix includes negative values"); return(NA)}
